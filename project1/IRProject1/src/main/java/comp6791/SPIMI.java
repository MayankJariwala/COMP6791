...
public class SPIMI {
    ...
    public static void main(String[] args) {
        ...
        SPIMI.startReadingFiles();
        Scanner scanner = new Scanner(System.in);

        System.out.println("Do you want to start merging phase?");
        String answer = scanner.nextLine().toLowerCase();
        while (!answer.equalsIgnoreCase("yes")) {
            answer = scanner.nextLine();
        }
        SPIMI.startMergingPhase();
        System.out.println("Do you want to start firing query (yes/no)?");
        String stopTrigger = scanner.nextLine();
        while (stopTrigger.equalsIgnoreCase("yes")) {
            String results = new Query(SPIMI.searchPointers).queryBoxer();
            System.out.println(results);
            System.out.println("Another Query (yes/no)?");
            stopTrigger = scanner.nextLine();
        }
        System.gc();
        scanner.close();
    }
                ...
                while (corpusRead < docsToRead) {
                    ...
                    if (bodyContent.length == 2 && !bodyContent[1].trim().equals("")) {
                        String[] cleanContentTokens = bodyContent[1]
                                ...
                                .trim()
                                .split(" ");
                        tokens = Arrays.stream(cleanContentTokens).filter(s -> !s.equalsIgnoreCase(""))
     ...
     */
    private void createEntryInSearchPointerList(SortedMap<String, SortedSet<Integer>> invertedDict, String invertedIndexFileName) {
